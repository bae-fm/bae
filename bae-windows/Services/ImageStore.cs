...
internal sealed class ImageStore
{
    ...
    private readonly ImageCache<BitmapImage> _cache = new(DecodedByteCost);
    ...
    internal BitmapImage? Cached(ImageContent content, int pixelWidth)
    {
    ...
    /// </summary>
    internal async Task<BitmapImage?> LoadAsync(ImageContent content, int pixelWidth)
    {
        var key = CacheKey(content, pixelWidth);
        ...
        var bytes = await Task.Run(() => ReadBytes(content)).ConfigureAwait(true);
        var bitmap = bytes is null ? null : Decode(bytes, pixelWidth);
    ...
    /// control's <see cref="Microsoft.UI.Xaml.FrameworkElement.Tag"/> at bind time
    /// and the reply is applied
    ...
    /// bytes are corrupt or an unsupported format.
    ///
    /// A <see cref="BitmapImage"/> is a DependencyObject, so unlike the other
    /// stacks this step runs on the UI thread; only the byte read is moved off
    /// it. <c>DecodePixelWidth</c> still keeps the decode itself small.</summary>
    private static BitmapImage? Decode(byte[] bytes, int pixelWidth)
    {
        try
        {
            using var stream = new MemoryStream(bytes);
            var bitmap = new BitmapImage { DecodePixelWidth = pixelWidth };
    ...
    /// <summary>What a decode is budgeted at. A <see cref="BitmapImage"/> reports
    /// its pixel dimensions only once it has finished decoding, which happens
    /// after this returns, so the budget uses the square bound of the width the
    /// decode was asked for — an over-estimate for anything that isn't square,
    /// never an under-estimate.</summary>
    private static int DecodedByteCost(BitmapImage bitmap) =>
        Math.Max(1, bitmap.DecodePixelWidth * bitmap.DecodePixelWidth * 4);
}
...
internal sealed class ImageBinding(ImageContent? content, int pixelWidth)
{
    ...
    internal void Attach(ImageStore store, DispatcherQueue dispatcher)
    {
        _store = store;
