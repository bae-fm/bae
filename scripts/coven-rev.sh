...
# bae-core/Cargo.toml. Single source for stamping builds — macOS Info.plist
...
rev=$(grep -E '^coven = .*rev = "' bae-core/Cargo.toml | head -1 | sed -E 's/.*rev = "([0-9a-f]+)".*/\1/')
if [[ -z "${rev:-}" || "$rev" == coven* ]]; then
  echo "coven rev not found in bae-core/Cargo.toml" >&2
  exit 1
fi
echo "${rev:0:7}"
