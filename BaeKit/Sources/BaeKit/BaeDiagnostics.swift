...
/// startup (from compiled-in values only) and required by `initKeyring` and
/// `BridgeHost`, so telemetry exists before anything that could fail. Local logging
...
public enum BaeDiagnostics {
    ...
    /// Call once at startup, before `initKeyring` and `BaeHost.make`. Infallible:
