...
- **`source_release_payloads`**: archived raw JSON from MusicBrainz and Discogs, keyed by provider release rather than by a local release. It supports exact reset and re-interpretation without changing the synced catalog graph.
