...
The top metadata slot displays either the draft or the Find online page. Opening or closing that page does not rewrite the draft. Applying a source — a release picked there, or the candidate's own file metadata through "Reset to file metadata" — replaces the draft, its provenance, and its selected source cover in one database transaction while retaining the candidate's file roles, CUE bindings, disc assignments, and other mapping decisions.
