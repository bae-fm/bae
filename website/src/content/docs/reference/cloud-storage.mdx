...
|---|---|---|
| S3-compatible | Access key + secret | Credentials sealed to the approved device during pairing |
| Google Drive | OAuth (PKCE) | New device's account approved, then the folder is shared |
| Dropbox | OAuth (PKCE) | New device's account approved, then the folder is shared |
| OneDrive | OAuth (PKCE) | New device's account approved, then the folder is shared |
| iCloud (CloudKit) | Apple ID, no sign-in | CloudKit share accepted during pairing |
