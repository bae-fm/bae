---
title: Storage Overview
description: How bae stores your music files
---

Release files can live in up to three places. When you import a release, you choose between **managed** and **unmanaged**. Managed files are copied into bae's storage; unmanaged files stay where they are.

## At a Glance

|                              | Unmanaged        | Managed — local    | Managed — cloud    |
|------------------------------|------------------|--------------------|--------------------|
| Files live at                | Your location    | `~/.bae/libraries/` | Cloud provider    |
| bae copies files on import   | No               | Yes                | Yes                |
| Available offline            | Yes              | Yes                | No                 |
| Encrypted at rest            | No               | No                 | Opaque home only   |
| Can sync to cloud            | No               | Yes                | —                  |
| Syncs to other devices       | No               | No                 | Yes                |
| Delete release removes files | No               | Yes                | Yes                |

A managed release can have files locally, in the cloud, or both. If you have a cloud home configured, managed imports go to both.

## Unmanaged

Point bae at files you already have organized. bae indexes them but doesn't move, copy, or touch anything. If you already have terabytes of music arranged your way, this lets you catalog it without duplicating a single byte.

The tradeoff: bae can't encrypt, sync, or back up unmanaged files.

## Managed — local

Every library has a home directory on your machine (`~/.bae/libraries/{uuid}/`). This is where bae keeps its database, images, and — for managed releases — a local copy of the release files.

When you import a release as managed, bae copies the files into the library home. Your originals are untouched. The local copy is what bae plays from.

## Managed — cloud

A cloud home is a single remote storage location that holds your release files, metadata changesets, artwork, and snapshots. Setting one up enables multi-device sync and cloud backup.

When you import a managed release with a cloud home configured, bae copies files to the library home *and* uploads them to the cloud home. Both happen at import time.

A release that only exists in the cloud home (e.g., synced from another device that hasn't downloaded it yet) is not available for offline playback until the files are pulled locally.

### Opaque or browsable

When you set up the cloud home you choose how it stores objects, for any provider:

- **Opaque** (default) encrypts every object with the library key and stores it at an obfuscated path. The provider sees only ciphertext.
- **Browsable** stores everything unencrypted with real filenames intact, under stable id-keyed paths — `storage/{album_id}/{release_id}/{import_folder}/01 Track.flac` for release files (mirroring the imported folder), `images/{album_id}/{release_id}/cover.jpg` for artwork — so you can open the bucket and play or copy your actual files with any tool. The folders are ids (not artist/album text), which keeps the layout collision-free and each release's folder immutable. Nothing is encrypted; the home relies on the provider's access control. Browsable is not public — the bucket's credentials gate it either way.

See [Encryption](/architecture/encryption#storage-modes-opaque-and-browsable) for the full comparison.

### Supported providers

- **Google Drive** — sign in with OAuth, bae creates a folder in your Drive
- **Dropbox** — sign in with OAuth
- **OneDrive** — sign in with OAuth
- **iCloud Drive** (macOS) — uses your iCloud container, no sign-in needed
- **S3-compatible** — any bucket (AWS S3, Backblaze B2, Cloudflare R2, Wasabi, MinIO, etc.)

Consumer cloud providers (Google Drive, Dropbox, etc.) use OAuth sign-in — no API keys or bucket configuration. iCloud Drive (macOS only) uses your existing iCloud container with no sign-in. S3-compatible providers require manual credential setup and are aimed at more technical users.

## How It Works

1. Import a release — choose **managed** or **unmanaged**
2. Managed imports copy files to the library home (local)
3. If a cloud home is configured, managed imports also upload files there — encrypted on an opaque home, in the clear on a browsable one
4. Other devices connected to the same cloud home sync automatically

See [Sync](/storage/sync) for how multi-device sync works and [Encryption](/architecture/encryption) for how files are encrypted.
