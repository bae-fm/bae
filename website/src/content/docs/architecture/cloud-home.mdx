...
The cloud home is a single remote storage location for a library -- one Google Drive folder, one S3 bucket, one Dropbox folder. It holds release files, metadata changesets, artwork, snapshots, and the membership chain. On an **opaque** home (the default) all trust lives in cryptography, not the storage backend -- the provider stores ciphertext at obfuscated keys. A home can instead be set to **browsable**, storing everything unencrypted at readable paths; see [Storage modes](/architecture/encryption#storage-modes-opaque-and-browsable).
...
- **Cloud home layout** -- `changes/`, `heads/`, `snapshot.db`, `images/`, `storage/`, `membership/`, `keys/`. Same logical paths regardless of backend. An opaque home shards audio and artwork under `storage/` and `images/` by a hash of their id; a browsable home lays them out at readable paths under the same two prefixes (`storage/Artist/Album/01 Track.flac`, `images/Artist/Album/cover.jpg`).
