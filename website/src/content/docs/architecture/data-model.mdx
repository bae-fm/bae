...
A library can optionally have a **cloud home** -- a single cloud location (Google Drive folder, S3 bucket, etc.) that acts as the hub for multi-device sync. One cloud home per library, configured in `config.yaml`. See [Cloud Home](/architecture/cloud-home) for details.
...
Whatever files came with a release: audio, images, CUE sheets, logs, etc. These are the user's data. bae stores them exactly as imported, preserving the original files so they can be exported intact.
