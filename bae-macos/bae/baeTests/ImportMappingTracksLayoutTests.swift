...
private struct SheetAssignmentChange: Equatable {
    let sheet: String
...
extension ImportMappingTracksLayoutTests {
    @MainActor
    @Test(
        "each CUE FILE menu assigns and clears only its own reference",
    ...
    func cueReferenceCommands(editable: Bool, referenceIndex: Int) async throws
    {
        ...
        #expect(buttons.allSatisfy { $0.isEnabled == editable })
        guard editable else {
            #expect(recorder.sheetBindings.isEmpty)
            return
        }
        let button = buttons[referenceIndex]
    ...
    @MainActor
    @Test("a partial CUE shows its missing reference beside the assigned one")
    func partialCueAssignments() async throws {
        ...
        let menus = SnapshotTestSupport.descendants(of: host)
            .compactMap { $0 as? NSPopUpButton }
        try #require(menus.count == 2)
        SnapshotTestSupport.populateMenu(menus[1])
        let menu = try #require(menus[1].menu)
        #expect(
            menu.items.contains { $0.title == "Second.flac" && $0.isEnabled }
        )
        withExtendedLifetime(window) {}
    }
...
extension ImportMappingTracksLayoutTests {
    @MainActor
    @Test(
        "multi-file CUE assignments are visible without opening a menu",
        arguments: [(true, true), (true, false), (false, false)]
    )
    func cueAssignmentsRemainVisible(editable: Bool, hasOffers: Bool)
        async throws
    {
