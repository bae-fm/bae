    ...
    func importReadyCandidates(_ keys: [String]) {
        ...
        for key in keys {
            guard
                ...
                // faster sibling call, or reclassified) — the list content
            ...
            do {
                try importer.startImport(
                    ...
                    storageMode,
                    storagePinned,
                    // Core derives this from the evidence that matched: a disc
                    // ID that named one release claims the pressing, a barcode
                    // or a text hit claims only the album. A bulk import has no
                    // claim line to read, so it commits what the row carries.
                    matched.claim,
                    nil
                )
            }
