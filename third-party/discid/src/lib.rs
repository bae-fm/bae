...
// The MSVC signedness casts in the Features bitflags (see below) are trivial on
// clang targets but necessary on MSVC, so this vendored copy doesn't deny
// trivial{,_numeric}_casts the way upstream does.
...
bitflags! {
    ...
    pub struct Features: u32 {
        ...
        /// will be read.
        // `.0` is the C enum's underlying integer: unsigned under clang, signed
        // under MSVC. Cast to the bitflags' u32 so this compiles on both.
...
impl From<Features> for discid_feature {
    fn from(item: Features) -> Self {
        // The C enum field is signed under MSVC, unsigned under clang; convert
        // from the u32 bits to whichever it is. The values are small flags.
