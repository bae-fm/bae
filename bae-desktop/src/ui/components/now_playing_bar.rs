...
use crate::ui::display_types::{playback_display_from_state, track_from_db_ref};
use crate::ui::use_library_manager;
use crate::ui::{image_url, Route};
use bae_core::db::DbTrack;
...
pub fn NowPlayingBar() -> Element {
    ...
    let track = use_memo(move || match state() {
        PlaybackState::Playing { ref track, .. } | PlaybackState::Paused { ref track, .. } => {
            Some(track_from_db_ref(track))
        }
        _ => None,
    });

    let playback_display = use_memo(move || playback_display_from_state(&state()));
