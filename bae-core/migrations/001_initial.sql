...
CREATE TABLE IF NOT EXISTS releases (
    ...
    -- lives in `release_unmanaged_source` / `file_cache`, NOT here — it must
    -- not sync.
...
-- DEVICE-LOCAL: the user's own files for an unmanaged release, in place, in a
-- folder they own. The ONLY copy of the audio (not a cache) — the library
-- references the bytes, it does not own them. A row is present iff this device
-- holds the release as owned-in-place files, which is only meaningful while
-- `managed = 0` (an unmanaged release, or one mid-Manage whose upload hasn't
-- landed). Not synced (no `_updated_at`, absent from SYNCED_TABLES): one
-- device's local storage must never overwrite another's.
CREATE TABLE IF NOT EXISTS release_unmanaged_source (
    release_id          TEXT PRIMARY KEY REFERENCES releases (id) ON DELETE CASCADE,
    -- Folder the files live in on this device; a file is at
    ...
    -- upload succeeds; the upload observer reads this when the release's last
    -- upload lands, deletes the originals, then drops this row.
    delete_after_upload BOOLEAN NOT NULL DEFAULT 0
);

-- DEVICE-LOCAL, PER FILE: a local cache copy of a managed blob at
-- `storage/<file_id>`. `storage/` IS the cache — a row plus the file on disk is
-- a cache hit. `pinned` marks an entry eviction-exempt (and exempt from the
-- max-size accounting); a release reads as "Pinned" when every one of its files
-- has a `pinned = 1` row. Not synced — each device owns its own cache.
CREATE TABLE IF NOT EXISTS file_cache (
    file_id          TEXT PRIMARY KEY REFERENCES release_files (id) ON DELETE CASCADE,
    pinned           BOOLEAN NOT NULL,
    size_bytes       INTEGER NOT NULL,
    -- RFC3339; set on insert/pin (read-bumping arrives with eviction).
