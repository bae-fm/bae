...
async fn resizing_a_file_orphans_the_old_row_under_a_new_hash() {
    ...
    store_candidate_state(&db, &original, &host_root("/music/Some Album")).await;
    let row = new_candidate_row(
        current_candidate_as_read(&db, &original_hash).await,
        &host_root("/music/Some Album"),
        &sample_verdict(),
        2_700_000,
    );
...
async fn every_metadata_provenance_variant_survives_a_database_reopen() {
    ...
    for (content_hash, folder_path, size, provenance) in &cases {
        ...
        let row = new_candidate_row(
            current_candidate_as_read(&db, content_hash).await,
            folder_path,
            &sample_verdict(),
            1_000,
        );
...
async fn a_re_run_that_settles_elsewhere_drops_the_pick_its_own_earlier_verdict_made() {
    ...
    let mut settled = new_candidate_row(
        current_candidate_as_read(&db, &hash).await,
    ...
    assert_eq!(
        re_run.metadata.provenance, None,
        "nothing was found to pick"
    );
    crate::import::CandidatePreparations::new(db.clone())
...
async fn a_re_run_that_settles_elsewhere_replaces_the_pick_it_made() {
    ...
    let mut first = new_candidate_row(
        current_candidate_as_read(&db, &hash).await,
        &host_root("/music/Album"),
        &sample_verdict(),
        2_700_000,
    ...
    let mut second = new_candidate_row(
        current_candidate_as_read(&db, &hash).await,
        &host_root("/music/Album"),
        &sample_verdict(),
        2_700_000,
