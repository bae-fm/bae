...
/// A result that asks something — here, the folder's own lengths could not
/// be read to check the pick against — leaves the pane where the person left
/// it.
#[tokio::test(flavor = "multi_thread")]
async fn a_settled_pick_that_asks_something_leaves_the_pane_where_it_was() {
    ...
    fixture
        .store_settled_verdict(&dir, "mb-settled-2", "rg-settled-2", 0)
        .await;

    assert_eq!(
        fixture.classification_for(&dir).await,
        QueueClassification::NeedsYou(crate::identify::NeedsYou::LocalDurationUnknown)
    );
    assert_eq!(
        fixture
            .pane(&dir)
