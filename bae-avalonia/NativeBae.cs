...
internal static partial class NativeBae
{
    ...
    internal static (BridgeMakeReleasesRemoteOutcome? Outcome, string? Error) MakeReleasesRemote(
        AppHandle handle,
        IReadOnlyList<string> releaseIds,
        bool pin) =>
        CaptureBridgeValue(() =>
            Await(() => handle.MakeReleasesRemote(releaseIds.ToArray(), pin)));
    ...
    internal static Settings GetSettings(AppHandle handle) =>
        SettingsFromConfig(handle.GetConfig());
    ...
    /// <summary>Provider art at a URL for the import flow's cover search, or
    /// null when the source serves no image there and on a failed fetch
    /// (logged). Core owns the socket; the UI never opens one.</summary>
    internal static byte[]? RemoteImage(AppHandle? handle, string url) =>
